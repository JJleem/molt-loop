...
- Task는 한 번에 구현하고 검증할 수 있는 기능 단위로 나눈다. 파일마다 별도 Task를 만들지 않는다.
- 같은 인터페이스를 함께 수정해야 하는 작은 변경은 묶고, 별도 실패 원인이 있는 큰 기능은 나눈다.
- 서로 의존하지 않는 기능에 불필요한 depends_on을 만들지 않는다.
- request에는 관련 코드의 실제 경로와 진입점을 짧게 넣는다. 소스 전문이나 조사 과정을 복사하지 않는다.
- 사람이 확인해야 하는 AC를 관습적으로 넣지 않는다. 실제 제품 선택이 필요한 질문만 모아서 반환한다.
- 모든 AC가 기존 결정론적 Gate로 충분히 판정되는 작은 변경에만 requires_verifier: false를 제안한다.
  기능·보안·아키텍처의 의미를 판단해야 한다면 독립 검증을 유지한다.
